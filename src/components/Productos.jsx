...
function Productos() {
  ...
  async function importarExcel(e) {
    ...
    reader.onload = async (evt) => {
      ...
      if (nuevos.length === 0) return alert('No se encontraron productos en el archivo')
    ...
    <div className="flex-1 flex flex-col">
      <div className="bg-stone-100 border-b border-gray-200 px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <label className="border border-gray-200 text-gray-500 text-xs px-4 py-2 rounded-lg cursor-pointer hover:bg-gray-50">
            📥 Importar Excel
            <input
              type="file"
              accept=".xlsx,.xls"
      ...
      <div className="p-5 flex flex-col gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setNuevoProducto({ ...nuevoProducto, precio_lista: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg pl-7 pr-3 py-2 text-sm outline-none focus:border-gray-400"
                    placeholder="0"
                  />
                </div>
              </div>
        ...
        {editando && (
          <div className="bg-stone-100 border-2 border-gray-900 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="number"
                    value={editando.precio_lista}
                    onChange={(e) => setEditando({ ...editando, precio_lista: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg pl-7 pr-3 py-2 text-sm outline-none focus:border-gray-400"
                  />
                </div>
              </div>
              ...
              <div className="col-span-2 flex items-center gap-3 pt-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <div
                    ...
                    className={`w-10 h-5 rounded-full transition-colors cursor-pointer ${editando.activo ? 'bg-gray-900' : 'bg-gray-200'}`}
                  >
                    <div className={`w-4 h-4 bg-stone-100 rounded-full mt-0.5 transition-transform ${editando.activo ? 'translate-x-5' : 'translate-x-0.5'}`}></div>
                  </div>
            ...
            <div className="flex gap-2">
              <button onClick={guardarEdicion} className="bg-gray-900 text-white text-xs px-4 py-2 rounded-lg">Guardar cambios</button>
        ...
        <div className="flex gap-3">
          ...
          >
            {categorias.map(cat => (
              <option key={cat} value={cat} className="capitalize">{cat === 'todas' ? 'Todas las categorías' : cat}</option>
            ))}
          </select>
        </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {productosFiltrados.length === 0 ? (
                  <tr><td colSpan="6" className="text-center py-12 text-gray-300">No hay productos todavía</td></tr>
                ) : (
                    ...
                    return (
                      <tr key={producto.id} className={`border-b border-gray-50 hover:bg-gray-50 ${!producto.activo ? 'opacity-50' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-right flex items-center justify-end gap-2">
                          <button
                            onClick={() => { setEditando(producto); setMostrarForm(false) }}
